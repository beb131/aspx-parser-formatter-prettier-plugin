...
"<!--"                                      this.begin("htmlcomment"); return "<!--";
"<%"                                        this.begin("expr"); return "<%";
"<%="                                       this.begin("expr"); return "<%=";
"<%@"                                       this.begin("expr"); return "<%@";
"<%#"                                       this.begin("expr"); return "<%#";
"<script"                                   this.begin("expr"); return "<script";
{Text}                                      return "TEXT";

<htmlcomment>"-->"                          this.popState(); return "-->";
<htmlcomment>((?!\-\-\>)(.|$|\r\n|\r|\n))*  return "HTMLCOMMENT";
...
<html>"!DOCTYPE html"                       return "DOCTYPE";
<html>"INPUT"|"input"                       return "INPUT";
<html>"BR"|"br"                             return "BR";
<html>"HR"|"hr"                             return "HR";
<html>"LINK"|"link"                         return "LINK";
<html>"META"|"meta"                         return "META";
<html>"IMG"|"img"                           return "IMG";
<html>"P"|"p"                               return "P";
...
<html>(\")                                  return "QUOTE";
<html>"/"                                   return "/";
<html>{AttributeText}                       return "TEXT";
...
<expr>"</script>"                           this.popState(); return "</script>";
...
<expr>"new"                                 return "NEW";
<expr>"of"                                  return "OF";
<expr>"return"                              return "RETURN";
<expr>"switch"                              return "SWITCH";
<expr>"this"                                return "THIS";
<expr>"throw"                               return "THROW";
<expr>"try"                                 return "TRY";
<expr>"typeof"                              return "TYPEOF";
<expr>"void"                                return "VOID";
<expr>"while"                               return "WHILE";
<expr>"with"                                return "WITH";
<expr>"true"                                return "TRUE";
<expr>"false"                               return "FALSE";
<expr>"null"                                return "NULL";
<expr>"class"                               return "CLASS";
<expr>"const"                               return "CONST";
<expr>"enum"                                return "ENUM";
<expr>"export"                              return "EXPORT";
<expr>"extends"                             return "EXTENDS";
<expr>"import"                              return "IMPORT";
<expr>"int"                                 return "INT";
<expr>"long"                                return "LONG";
<expr>"float"                               return "FLOAT";
<expr>"double"                              return "DOUBLE";
<expr>"bool"                                return "BOOL";
<expr>"char"                                return "CHAR";
<expr>"string"                              return "STRING";
<expr>"object"                              return "OBJECT";
<expr>"var"                                 return "VAR";
<expr>"const"                               return "CONST";
<expr>"let"                                 return "LET";
<expr>"System"                              return "SYSTEM";
<expr>"Data"                                return "DATA";
<expr>"DataRow"                             return "DATAROW";
<expr>"DataTable"                           return "DATATABLE";
<expr>"DataSet"                             return "DATASET";
<expr>"Int32"                               return "INT32";
...
HTMLTag
    : "<!--" HTMLCOMMENT "-->"
    | "<" SelfCloseTag AttributeList ">"
    | "<" SelfCloseTag AttributeList "/" ">"
    | "<" IDENTIFIER AttributeList "/" ">"
    | "<" IDENTIFIER ":" IDENTIFIER AttributeList "/" ">"
    | "<" IDENTIFIER AttributeList ">" "<" "/" IDENTIFIER ">"
        {
            if ($2 != $7) {
                throw new SyntaxError(
                    "Syntax error on line " + (yylineno + 1) + ":\n" +
                    "Tag identifiers should be same (<" + $2 + "> does not match </" + $7 + ">)"
                );
            }
        }
    | "<" IDENTIFIER AttributeList ">" ElementList  "<" "/" IDENTIFIER ">"
        {
            if ($2 != $8) {
                throw new SyntaxError(
                    "Syntax error on line " + (yylineno + 1) + ":\n" +
                    "Tag identifiers should be same (<" + $2 + "> does not match </" + $8 + ">)"
                );
            }
        }
    ;
...
ASPBlock
    : "<%" Statement "%>"
    | "<%=" Statement "%>"
    | "<%@" Statement AttributeList "%>"
    | "<%#" Statement "%>"
    ;

JSBlock
    : "<script" AttributeList ">" Statement "</script>"
    | "<script" AttributeList ">" ASPBlock "</script>"
    | "<script" AttributeList ">" Statement ASPBlock "</script>"
    ;
...
AttributeValue
    ...
    | TEXT AttributeValue
    | ExpressionStatement AttributeValue
    | "<%=" Statement "%>"
    // | StringLiteral
    ;
...
IfStatement
    :  IF "(" Expression ")" "{" ElementList "}"
    |  IF "(" Expression ")" "{" ElementList "}" ELSE "{" ElementList "}"
    |  IF "(" Expression ")" "{" ElementList "}" ELSE "{" "%>" ElementList "<%" "}"
    ;

ForStatement
    :  FOR "(" LexicalDeclaration Expression ";" PostfixExpression ")" "{" ElementList "}"
    |  FOR "(" LexicalDeclaration Expression ";" PostfixExpression ")" "{" "%>" ElementList "<%" "}"
    ;
...
MemberExpression
    ...
    | PrimaryExpression
    | FunctionExpression
    | MemberExpression MemberOperator
...
LexicalDeclaration
    : DeclarationKeyword IDENTIFIER
    | DeclarationKeyword IDENTIFIER "=" MemberExpression
    | DeclarationKeyword IDENTIFIER "=" MemberExpression ";"
    ;
